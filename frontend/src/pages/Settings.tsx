...
export function SettingsPage() {
  const { state, toggleTheme, updateUser, deleteAccount } = useApp();
  ...
  return (
    <>
      <div className="animate-fade-in flex gap-8 items-start">
        ...
        {/* ── Left column: fills remaining space, no width cap ─────────────── */}
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  style={{
                    ...
                    height: 24,
                    borderRadius: 12,
                    backgroundColor: isDark ? "#8BA888" : "#E8E4DD",
                    border: "none",
            ...
            <div className="rounded-2xl border border-border dark:border-border-dark bg-white dark:bg-[#252523] px-6 py-5 flex flex-col gap-5">
              {(picture || state.user?.picture) && (
                <div className="flex items-center gap-3">
                  <img
                    src={picture || state.user?.picture}
                    alt="Profile"
              ...
              <div className="flex items-center gap-3">
                ...
                {profileSuccess && (
                  <span className="font-sans text-xs text-sage-700 dark:text-sage-300 animate-fade-in">
                    ✓ saved
                  </span>
                )}
                {profileError && (
                  <span className="font-sans text-xs text-rose-400 animate-fade-in">
                    {profileError}
                  </span>
                )}
              </div>
        ...
        {/* ── Right column: fixed width, mirrors dashboard quote panel ─────── */}
        <div className="hidden lg:flex flex-col gap-6 w-64 shrink-0 mt-[76px]">
          ...
          <div>
            ...
            <div className="rounded-2xl border border-border dark:border-border-dark bg-white dark:bg-[#252523] px-5 py-5 flex flex-col gap-3">
              <p className="font-sans text-xs text-ink-muted dark:text-zinc-400 leading-relaxed">
                Export all your journal entries as a plain text file. Your entries are decrypted locally before download.
              </p>
              <button
          ...
          {/* Danger zone */}
          <div>
            <h3 className="font-sans text-xs uppercase tracking-widest text-rose-400/70 mb-4">
              Danger zone
            </h3>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative z-10 w-full max-w-sm rounded-2xl border border-rose-200 dark:border-rose-900/60 bg-white dark:bg-[#252523] shadow-xl px-7 py-7 animate-fade-in">
            <h3 className="font-serif text-lg italic text-ink dark:text-zinc-100 mb-2">
              Delete account
            </h3>
            ...
            />
            {deleteError && (
              <p className="font-sans text-xs text-rose-500 mb-3 animate-fade-in">
                {deleteError}
              </p>
            )}
            <div className="flex gap-3">
              <button
                ...
                disabled={deleting}
                className="flex-1 rounded-full border border-border dark:border-border-dark px-4 py-2 font-sans text-sm text-ink-muted dark:text-zinc-400 hover:text-ink dark:hover:text-zinc-200 transition disabled:opacity-40"
              >
                cancel
              </button>
              <button
